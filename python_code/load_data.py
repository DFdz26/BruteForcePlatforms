...
def random_generate_data(required_items, data=None):
    if data is None:
        data = data_arduino
    ret = []
    ...
    for _ in range(required_items):
        ...
        if print_data_debug:
            print("Chosen:", segment1, "Inflation time (ms):",  inflation_time,
                  "" if inflation_time_filtered == inflation_time else f"Filtered value: {inflation_time_filtered}")
            print("Chosen:", segment2, "Chamber:", chamber)
            print("Chosen:", segment3, "Iterations:", iterations)
            print("------------")
